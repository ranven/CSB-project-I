...
def get_all_posts(user_id, sort_param, search_term):
    if user_id == 0:
        ...
        (SELECT COALESCE(SUM(CASE WHEN v.vote_code = 1 THEN 1 WHEN v.vote_code = 0 THEN -1 ELSE 0 END), 0) FROM votes v WHERE v.post_id = p.post_id) as votes,
    ...
    try:
        result = db.session.execute(
            sql, {"user_id": user_id, "search_term": search_term})
...
def get_post(user_id, post_id):
    ...
    try:
        result = db.session.execute(
            sql, {"user_id": user_id, "post_id": post_id})
...
def submit_post(title, content, user_id):
    ...
    db.session.execute(
        sql, {"title": title, "content": content, "user_id": user_id})
    db.session.commit()
    return True
...
def get_order_clause(sort_param):
    ...
    match sort_param:
        case "new":
            order += "p.edited_at DESC, p.created_at DESC"
        case "old":
            order += "p.edited_at ASC, p.created_at ASC"
        case "votes":
            order += "votes DESC"
        case "comments":
...
def get_search_clause(search_term):
    if len(search_term) > 0:
        search = "WHERE lower(p.title) LIKE '%' || lower(:search_term) || '%' OR lower(p.content) LIKE '%' || lower(:search_term) || '%'"
    else:
        search = ''
    return search
